...
public final class Observer<Value, Error: Swift.Error> {
	...
	///
	/// - parameters:
	///   - context: The context of the observer.
	///   - action: A closure to lift over received event.
	internal init(context: ObserverContext, _ action: @escaping Action) {
...
internal enum ObserverContext {
	case anonymous
	case eventEmitter((SignalDownstreamEvent) -> Void)
}

internal enum SignalDownstreamEvent {
	case interrupted
	// case deactivated
	// case activated
}
